...
public class ApoButtonEditIcon extends ApoButton {

    private final float[] circleColor;
    private final float[] iconColor;

    public ApoButtonEditIcon(int x, int y, int size, String function, float[] circleColor, float[] iconColor) {
    ...
    private void drawPencilFilled(GameScreen screen, float cx, float cy) {
        ...
        // Pencil oriented from bottom-left (tip) to top-right (eraser)
        // Scale factor based on button size
        float s = getWidth() / 48f;

        // Pencil body (rotated rectangle via rectLine)
        float bodyX1 = cx - 6 * s;
        float bodyY1 = cy + 6 * s;
        ...
        float tipX = cx - 12 * s;
        float tipY = cy + 12 * s;
        // Two corners where the body starts
        float perpX = 3f * s;
        float perpY = 3f * s;
        ...
        // Eraser end (small rectangle across the top)
        ...
        // Eraser divider line - slightly different shade
