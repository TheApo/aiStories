...
public class ApoButtonSwitch extends ApoButtonColor {
    ...
    public void renderText(GameScreen screen, int changeX, int changeY, String llm) {
        if (isSelect()) {
            screen.drawString(llm, this.getX() + changeX + this.getWidth() + 5, this.getY() + changeY + this.getHeight() / 2, Constants.COLOR_WHITE, getFont(), DrawString.BEGIN, false, true);
        } else {
            screen.drawString(llm, this.getX() + changeX - 5, this.getY() + changeY + this.getHeight() / 2, Constants.COLOR_WHITE, getFont(), DrawString.END, false, true);
        }
