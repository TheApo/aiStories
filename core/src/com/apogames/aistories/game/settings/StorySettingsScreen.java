...
public class StorySettingsScreen extends SequentiallyThinkingScreenModel {
    ...
    public void init() {
        ...
        if (this.promptField == null) {
            this.promptField = new Textfield(PROMPT_X, PROMPT_Y, PROMPT_WIDTH, PROMPT_HEIGHT, AssetLoader.font15);
    ...
    public void keyButtonReleased(int keyCode, char character) {
        ...
        if (promptField.isSelect()) {
            if (promptField.handleNavigationKey(keyCode)) {
                return;
            }
            promptField.addCharacter(keyCode, character);
        }
    }
